<!-- Note that in v-for for the first button, :key=entry.id
---- and v-for in second button uses :key=entry.name
---- Not sure how this changes things, but pointing it out
---- just in case.
-->
<template>
     <div class="container">
        <button v-for="entry in accordionData" :key=entry.id
        ...
        <span 
            ...
            title="Clicking 'Toggle Both Cards' while one is activated will turn off one and turn on the other one.
            To put both on the same page, you have to decide to click on the card to toggle one or the other so both
            are either activated or deactivated at the same time.">
        ...
        <div v-for="entry in accordionData" :key=entry.name
            :id="entry.id"
